...
# Reroject CRS and buffer
nad83 <- 4269 # crs based in meters
wgs84 <- 4326 # crs based in lat, lon

lasan_ggla_nad83 <- st_transform(lasan_ggla_rough, crs = nad83)

lasan_ggla_buffered <- st_buffer(lasan_ggla_nad83, dist = 5)

# Create full LASAN_GGLA area (minus KYCC) 
lasan_ggla_unioned <- st_union(lasan_ggla_buffered)

lasan_ggla_wgs84 <- st_transform(lasan_ggla_unioned, crs = wgs84)

# Create LASAN_GGLA area w/ no np overlap
lasan_ggla <- st_difference(lasan_ggla_wgs84, all_np_grants)


# Read in LASAN_TREE
lasan_tree_rough <- st_read(dsn = "data_general/LASAN_SHADE-LA_TREEmendous.kml") %>% 
  select("Name", "geometry") %>% 
...
lasan_tree_nad83 <- st_transform(lasan_tree_rough, crs = nad83)

lasan_tree_buffered <- st_buffer(lasan_tree_nad83, dist = 5)

# Create full LASAN_TREE area (minus KYCC) 
lasan_tree_unioned <- st_union(lasan_tree_buffered)

lasan_tree_wgs84 <- st_transform(lasan_tree_unioned, crs = wgs84)

# Create LASAN_GGLA area w/ no np overlap
lasan_tree <- st_difference(lasan_tree_wgs84, all_np_grants_unioned)


tm_shape(lasan_ggla) +
  tm_polygons(col = "goldenrod", alpha = 0.5) +
  tm_shape(lasan_tree) +
