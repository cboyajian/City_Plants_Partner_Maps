...
# Save to KML File
st_write(lasan_ggla_unioned, "grant_area_data/LASAN_GGLA_minusKYCC.kml", append = FALSE)

st_write(lasan_tree_unioned, "grant_area_data/LASAN_TREEandSHADE.kml", append = FALSE)

### THEN import into Google maps, export out of Google maps, and re-save over old file ###
```

```{r}
# Import LASAN_GGLE file exported from Google maps
lasan_ggla_gmaps <- st_read("grant_area_data/LASAN_GGLA_minusKYCC.kml")

lasan_ggla_minus_city <- st_intersection(lasan_ggla_gmaps, la_city_outline)

lasan_ggla <- st_difference(lasan_ggla_minus_city, all_np_grants_unioned) %>% 
  st_make_valid() %>% 
  select("Name", "geometry") %>% 
  mutate(Name = "LASAN-GGLA")
  

# Import LASAN_TREE file exported from Google maps
...
lasan_tree <- st_difference(lasan_tree_minus_city, all_np_grants_unioned) %>% 
  st_make_valid()%>% 
  select("Name", "geometry") %>% 
